 ...
 */
export function validateMaintenanceConfig(data: unknown): data is MaintenanceConfig {
 ...
 */
export function isInMaintenanceWindow(config: MaintenanceConfig): boolean {
    const now = new Date();
...
export function readMaintenanceConfig(
    filePath: string = "public/maintenance.json",
    ...
    try {
        ...
        if (!validateMaintenanceConfig(data)) {
            if (!silent) {
                console.warn(
                    `Invalid maintenance config structure in ${filePath}. Using default: { enabled: false }`
                );
            }
 ...
 */
export function getMaintenanceStatus(filePath?: string): MaintenanceStatus {
